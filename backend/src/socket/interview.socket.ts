...
type ClientMessage =
  ...
  | { type: "transcript"; questionId: string; text: string; durationSeconds: number }
  | { type: "interrupt" }
  | { type: "ping" };

export function attachInterviewSocket(server: any) {
...
async function handleMessage(socket: WebSocket, userId: string, message: ClientMessage) {
  if (message.type === "ping") return send(socket, { type: "pong" });
  if (message.type === "interrupt") return send(socket, { type: "ai_interrupted" });

  if (message.type === "join") {
    const interview: any = await Interview.findOne({ _id: message.interviewId, userId });
  ...
  if (message.type === "transcript") {
    ...
    const interview: any = await Interview.findOne({ "questions._id": message.questionId, userId });
    const question = interview?.questions.id(message.questionId);
    const feedback = await evaluateAnswer(question?.prompt ?? "", message.text);
    const currentIndex = interview.questions.findIndex((item: any) => item._id.toString() === message.questionId);
...
function authenticateSocket(request: IncomingMessage) {
