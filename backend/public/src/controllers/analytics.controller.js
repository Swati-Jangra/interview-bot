...
export const getDashboard = asyncHandler(async (req, res) => {
    ...
    res.json({
        ...
        recentInterviews,
        weakTopics: recentInterviews.flatMap((item) => item.summary?.weakTopics ?? []).slice(0, 5),
        strongTopics: recentInterviews.flatMap((item) => item.summary?.strongTopics ?? []).slice(0, 5)
    });
});
